...
(function () {
  ...
  function Gnb({ selected, onSelect }) {
    ...
    useEffect(() => {
      ...
      GNB.forEach((m) => m.items.forEach((it) => { if (it.thumb) { const i = new Image(); i.decoding = "sync"; i.src = it.thumb; } }));
    }, []);
    ...
    return (
      <header style={{ position: "sticky", top: 0, zIndex: 300, background: "rgba(58,58,58,.97)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", borderBottom: "1px solid rgba(255,255,255,.07)" }}>
        <div style={{ maxWidth: 1320, margin: "0 auto", height: 62, padding: "0 var(--gutter)", display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center" }}>
          ...
          <div style={{ justifySelf: "end", display: "flex", alignItems: "center", gap: 16, color: "#fff" }}>
            <div style={{ position: "relative", display: "flex", alignItems: "center", height: 62 }}
              onMouseEnter={() => setMyOpen(true)} onMouseLeave={() => setMyOpen(false)}>
              <span style={{ display: "flex", cursor: "pointer", color: myOpen ? "var(--ws-mint)" : "#fff", transition: "color var(--dur-fast) var(--ease-out)" }}>
                <Icon d={<><circle cx="12" cy="8" r="4" /><path d="M4 21c0-4 3.6-7 8-7s8 3 8 7" /></>} />
              </span>
              {myOpen &&
              <div style={{ position: "absolute", top: 58, right: -8, width: 196, background: "rgba(52,52,52,.97)", backdropFilter: "blur(18px)", WebkitBackdropFilter: "blur(18px)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 14, boxShadow: "0 34px 70px rgba(0,0,0,.45)", padding: "20px 0 18px", boxSizing: "border-box" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 12, padding: "0 22px 16px", borderBottom: "1px solid rgba(255,255,255,.12)" }}>
                  {[["로그인", "#fff"], ["회원가입", "var(--ws-mint)"]].map(([label, color]) =>
                    <button key={label} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "inherit", fontSize: 15, fontWeight: 700, letterSpacing: "-0.01em", color, textAlign: "center" }}>{label}</button>
                  )}
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 11, padding: "16px 22px 0" }}>
                  {["주문/배송 조회", "보유제품 관리", "쿠폰/적립금", "1:1문의", "회원정보"].map((label) =>
                    <button key={label}
                      onMouseEnter={(e) => { e.currentTarget.style.color = "var(--ws-mint)"; }}
                      onMouseLeave={(e) => { e.currentTarget.style.color = "rgba(255,255,255,.88)"; }}
                      style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "inherit", fontSize: 14, fontWeight: 500, letterSpacing: "-0.01em", color: "rgba(255,255,255,.88)", textAlign: "center", transition: "color var(--dur-fast) var(--ease-out)" }}>{label}</button>
                  )}
                </div>
              </div>}
            </div>
  ...
  function PageStage({ screen }) {
    ...
    const [pi, setPi] = useState(screen.startPage || 0);
